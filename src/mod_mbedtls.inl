...
static void mbed_debug(void* context, int level, const char* file, int line,
                       const char* str);
...
int mbed_sslctx_init(SSL_CTX* ctx, const char* crt) {
  ...
  rc = mbedtls_ctr_drbg_seed(&ctx->ctr, mbedtls_entropy_func, &ctx->entropy,
                             (unsigned char*)"CivetWeb", strlen("CivetWeb"));
  if (rc != 0) {
...
int mbed_ssl_accept(mbedtls_ssl_context** ssl, SSL_CTX* ssl_ctx, int* sock,
  ...
  *ssl = (mbedtls_ssl_context*)mg_calloc_ctx(1, sizeof(mbedtls_ssl_context),
                                             phys_ctx);
  if (*ssl == NULL) {
...
int mbed_ssl_write(mbedtls_ssl_context* ssl, const unsigned char* buf,
                   int len) {
...
static void mbed_debug(void* user_param, int level, const char* file, int line,
                       const char* str) {
  ...
  DEBUG_TRACE("mbedTLS DEBUG: file: [%s] line: [%d] str: [%s]", file, line,
              str);
}
