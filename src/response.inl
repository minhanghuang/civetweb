  ...
  while (conn->response_info.num_headers > 0) {
    ...
    mg_free(
        (void*)conn->response_info.http_headers[conn->response_info.num_headers]
            .name);
    conn->response_info.http_headers[conn->response_info.num_headers].name = 0;
    mg_free(
        (void*)conn->response_info.http_headers[conn->response_info.num_headers]
            .value);
    conn->response_info.http_headers[conn->response_info.num_headers].value = 0;
  }
#else
 ...
 *   -5:    out of memory
 */
int mg_response_header_add(struct mg_connection* conn, const char* header,
  ...
  if (value_len >= 0) {
    char* hbuf = (char*)mg_malloc_ctx((unsigned)value_len + 1, conn->phys_ctx);
    if (hbuf) {
