...
TIMER_API double timer_getcurrenttime(struct mg_context* ctx) {
#if defined(_WIN32)
...
TIMER_API int timer_add(struct mg_context* ctx, double next_time, double period,
...
TIMER_API int timers_init(struct mg_context* ctx) {
  /* Initialize timers data structure */
  ctx->timers = (struct ttimers*)mg_calloc_ctx(sizeof(struct ttimers), 1, ctx);
  ...
  /* Start timer thread */
  if (mg_start_thread_with_id(timer_thread, ctx, &ctx->timers->threadid) != 0) {
...
TIMER_API void timers_exit(struct mg_context* ctx) {
  if (ctx->timers) {
