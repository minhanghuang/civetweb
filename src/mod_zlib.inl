...
static void send_compressed_data(struct mg_connection* conn,
  ...
  zret = deflateInit2(&zstream, Z_BEST_COMPRESSION, Z_DEFLATED, MAX_WBITS | 16,
                      MEM_LEVEL, Z_DEFAULT_STRATEGY);

  if (zret != Z_OK) {
    mg_cry_internal(conn, "GZIP init failed (%i): %s", zret,
  ...
  if (zret != Z_STREAM_END) {
    ...
    mg_cry_internal(conn, "GZIP incomplete (%i): %s", zret,
                    (zstream.msg ? zstream.msg : "<no error message>"));
  }
...
static int websocket_deflate_initialize(struct mg_connection* conn,
                                        int server) {
  int zret = deflateInit2(
      &conn->websocket_deflate_state, Z_BEST_COMPRESSION, Z_DEFLATED,
      server ? -1 * conn->websocket_deflate_server_max_windows_bits
             : -1 * conn->websocket_deflate_client_max_windows_bits,
      MEM_LEVEL, Z_DEFAULT_STRATEGY);
  if (zret != Z_OK) {
    mg_cry_internal(
        conn, "Websocket deflate init failed (%i): %s", zret,
        (conn->websocket_deflate_state.msg ? conn->websocket_deflate_state.msg
                                           : "<no error message>"));
  ...
  if (zret != Z_OK) {
    mg_cry_internal(
        conn, "Websocket inflate init failed (%i): %s", zret,
        (conn->websocket_inflate_state.msg ? conn->websocket_inflate_state.msg
                                           : "<no error message>"));
...
static void websocket_deflate_negotiate(struct mg_connection* conn) {
  ...
  if (extensions && !strncmp(extensions, "permessage-deflate", 18)) {
    ...
    while (*extensions != '\0') {
      if (*extensions == ';' || *extensions == ' ')
        ...
        mg_cry_internal(conn, "Unknown parameter %s for permessage-deflate",
                        extensions);
        break;
      }
    }
...
static void websocket_deflate_response(struct mg_connection* conn) {
  if (conn->accept_gzip) {
    mg_printf(conn,
