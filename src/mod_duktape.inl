...
static const char* const civetweb_conn_id =
    "\xFF"
    "civetweb_conn";
static const char* const civetweb_ctx_id =
    "\xFF"
    "civetweb_ctx";
...
static void mg_duk_fatal_handler(duk_context* duk_ctx, duk_errcode_t code,
                                 const char* msg) {
...
static void mg_duk_v2_fatal(void* udata, const char* msg) {
...
static duk_ret_t duk_itf_write(duk_context* duk_ctx) {
  struct mg_connection* conn;
  duk_double_t ret;
  duk_size_t len = 0;
  ...
  */
  ...
  if (!conn) {
    duk_error(duk_ctx, DUK_ERR_ERROR,
...
static duk_ret_t duk_itf_read(duk_context* duk_ctx) {
  struct mg_connection* conn;
  char buf[1024];
  int len;
...
static duk_ret_t duk_itf_getoption(duk_context* duk_ctx) {
  struct mg_connection* conn;
  const char* ret;
  int optidx;
  duk_size_t len = 0;
...
static void mg_exec_duktape_script(struct mg_connection* conn,
  ...
  duk_ctx = duk_create_heap(mg_duk_mem_alloc, mg_duk_mem_realloc,
                            mg_duk_mem_free, (void*)conn->phys_ctx,
#if DUK_VERSION >= 20000L
                            mg_duk_v2_fatal
#else
  ...
  duk_put_prop_string(duk_ctx, -2, "request_method"); /* add string conn.r... */
  ...
  for (i = 0; i < conn->request_info.num_headers; i++) {
    ...
    duk_put_prop_string(duk_ctx, -2, conn->request_info.http_headers[i].name);
  }
  ...
  duk_put_prop_string(duk_ctx, -2, "civetweb"); /* call the table "civetweb" */
