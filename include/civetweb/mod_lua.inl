...
static int handle_lsp_request(struct mg_connection*, const char*,
                              struct mg_file*, struct lua_State*);

static void reg_lstring(struct lua_State* L, const char* name,
...
static void reg_llstring(struct lua_State* L, const void* buffer1,
                         size_t buflen1, const void* buffer2, size_t buflen2) {
...
static void reg_i64(struct lua_State* L, const char* name, int64_t val) {
  if (name == NULL) {
    return;
  }
  lua_pushstring(L, name);
...
static void reg_double(struct lua_State* L, const char* name, double val) {
  if (name != NULL) {
    lua_pushstring(L, name);
...
static void lua_cry(const struct mg_connection* conn, int err, lua_State* L,
  ...
  switch (err) {
    ...
    case LUA_YIELD:
      break;
    case LUA_ERRRUN:
      mg_cry_internal(conn, "%s: %s failed: runtime error: %s", lua_title,
                      lua_operation, lua_tostring(L, -1));
      break;
    case LUA_ERRSYNTAX:
      mg_cry_internal(conn, "%s: %s failed: syntax error: %s", lua_title,
                      lua_operation, lua_tostring(L, -1));
      break;
    case LUA_ERRMEM:
      mg_cry_internal(conn, "%s: %s failed: out of memory", lua_title,
                      lua_operation);
      break;
    ...
    case LUA_ERRGCMM:
      mg_cry_internal(conn, "%s: %s failed: error during garbage collection",
                      lua_title, lua_operation);
      break;
#endif
    case LUA_ERRERR:
      mg_cry_internal(conn, "%s: %s failed: error in error handling: %s",
                      lua_title, lua_operation, lua_tostring(L, -1));
      break;
    default:
      mg_cry_internal(conn, "%s: %s failed: error %i", lua_title, lua_operation,
                      err);
      break;
  }
}
...
static int lsp_sock_gc(lua_State* L) {
  int num_args = lua_gettop(L);
  size_t s;
  SOCKET* psock;
  ...
  if ((num_args == 1) && lua_istable(L, 1)) {
    ...
    if (s != sizeof(SOCKET)) {
      return luaL_error(
          L, "invalid internal state in __gc for object created by connect");
    }
    closesocket(*psock);
  } else {
...
static const char* lsp_kepler_reader(lua_State* L, void* ud, size_t* sz) {
  struct lsp_var_reader_data* reader = (struct lsp_var_reader_data*)ud;
  const char* ret;
  int64_t i;
  int64_t left;
  ...
  if ((reader->state == 2) || (reader->state == 3)) {
    ...
    for (;;) {
      ...
      close_tag_found =
          ((i + 1 < left) && (reader->begin[i + 1 + reader->consumed] == '>'));
...
static int run_lsp_kepler(struct mg_connection* conn, const char* path,
                          const char* p, int64_t len, lua_State* L, int depth) {
  int lua_ok;
...
static int run_lsp_civetweb(struct mg_connection* conn, const char* path,
  ...
  for (i = 0; i < len; i++) {
    ...
    if (((i + 1) < len) && (p[i] == '<') &&
      ...
      while (j < len) {
        ...
        if (((j + 1) < len) && (p[j] == lsp_mark_used) && (p[j + 1] == '>')) {
          ...
          mg_snprintf(conn, NULL, /* ignore truncation for debugging */
          ...
          if (is_var) {
            ...
            data.consumed = 0;
            data.tag = 0;
            lua_ok = mg_lua_load(L, lsp_var_reader, &data, chunkname, NULL);
          } else {
            /* For scripts: Execute them */
            lua_ok =
                luaL_loadbuffer(L, p + (i + 2 + s), j - (i + 2 + s), chunkname);
          }
  ...
  if ((file_name != NULL) && (num_args <= 2)) {
    ...
    if (include_history->depth >= ((int)(LSP_INCLUDE_MAX_DEPTH))) {
      mg_cry_internal(conn,
                      ...
                      (int)(LSP_INCLUDE_MAX_DEPTH), file_name);
    } else {
      ...
      if (*path_type == 'v') {
        ...
        (void)mg_snprintf(conn, &truncated, file_name_path,
                          sizeof(file_name_path), "%s/%s",
        ...
        if (!truncated) {
          ...
          (void)mg_snprintf(conn, &truncated, file_name_path + len,
                            sizeof(file_name_path) - len, "%s", file_name);
        }

      } else {
        return luaL_error(
            L, "invalid path_type in include(file_name, path_type) call");
      }
...
static int lsp_send_file_body(lua_State* L) {
  struct mg_connection* conn =
      (struct mg_connection*)lua_touserdata(L, lua_upvalueindex(1));
  int num_args = lua_gettop(L);
  const char* filename = (num_args == 1) ? lua_tostring(L, 1) : NULL;
...
/* mg.send_http_error */
static int lsp_send_http_error(lua_State* L) {
  struct mg_connection* conn =
      (struct mg_connection*)lua_touserdata(L, lua_upvalueindex(1));
  int num_args = lua_gettop(L);
  ...
  if ((status >= 100) && (status <= 999)) {
    ret = mg_send_http_error(conn, status, "%s",
...
/* mg.send_http_ok */
static int lsp_send_http_ok(lua_State* L) {
  struct mg_connection* conn =
      (struct mg_connection*)lua_touserdata(L, lua_upvalueindex(1));
  int num_args = lua_gettop(L);
  int type1, type2;
...
static int lsp_send_http_redirect(lua_State* L) {
  struct mg_connection* conn =
      (struct mg_connection*)lua_touserdata(L, lua_upvalueindex(1));
  int num_args = lua_gettop(L);
  int type1, type2;
...
/* mg.split_form_data */
static int lsp_split_form_urlencoded(lua_State* L) {
  int num_args = lua_gettop(L);
  const char* in;
  size_t len;
  char* buf;
...
static int lsp_base64_encode(lua_State* L) {
  ...
  if (num_args == 1) {
    ...
    if (text) {
      ...
      if (dst) {
        mg_base64_encode((const unsigned char*)text, (int)text_len, dst,
                         &dst_len);
        lua_pushstring(L, dst);
        mg_free(dst);
      } else {
...
/* mg.get_info */
static int lsp_get_info(lua_State* L) {
  int num_args = lua_gettop(L);
  int type1, type2;
  const char* arg1;
  double arg2;
...
/* mg.get_option */
static int lsp_get_option(lua_State* L) {
  int num_args = lua_gettop(L);
  int type1;
  const char* arg1;
  const char* data;
...
/* mg.trace */
static int lsp_trace(lua_State* L) {
  int num_args = lua_gettop(L);
  int arg_type[8];
  int trace_level = 0;
  int firstarg = 1;
...
/* mg.uuid */
static int lsp_uuid(lua_State* L) {
  union {
  ...
  if (num_args == 0) {
    ...
    sprintf(uuid_str,
            ...
            (unsigned)uuid.uuid_struct.data2, (unsigned)uuid.uuid_struct.data3,
...
static int lua_action_string(struct laction_string_arg* arg) {
  int err, ok;
  struct mg_context* ctx;
...
static int lua_action_funcref(struct laction_funcref_arg* arg) {
  int err, ok;
  struct mg_context* ctx;
...
static void lua_action_string_cancel(struct laction_string_arg* arg) {
  mg_free(arg);
}

static void lua_action_funcref_cancel(struct laction_funcref_arg* arg) {
...
static int lwebsocket_set_timer(lua_State* L, int is_periodic) {
  ...
  if (type1 == LUA_TSTRING) {
    ...
    if (0 == timer_add(ctx, delay, interval, 1, (taction)lua_action_string,
                       (void*)arg, (tcancelaction)lua_action_string_cancel)) {
    ...
    /* Create timer data structure and schedule timer */
    arg = (struct laction_funcref_arg*)mg_malloc_ctx(
        sizeof(struct laction_funcref_arg), ctx);
    if (!arg) {
      return luaL_error(L, "out of memory");
    }
    ...
    arg->pmutex = (ws ? &(ws->ws_mutex) : &(ctx->lua_bg_mutex));
    arg->funcref = funcref;
    if (0 == timer_add(ctx, delay, interval, 1, (taction)lua_action_funcref,
                       (void*)arg, (tcancelaction)lua_action_funcref_cancel)) {
      /* Timer added successfully */
      ok = 1;
    }
  } else {
...
/* mg.response.send() */
static int lsp_response_send(lua_State* L) {
  int http_status;
  int ret1, ret2, ret3;
...
static void push_lua_response_log_data(const struct mg_connection* conn,
                                       lua_State* L) {
  int i;
  const char* s;
  ...
  for (i = 0; i < conn->response_info.num_headers; i++) {
    reg_string(L, conn->response_info.http_headers[i].name,
...
static void prepare_lua_request_info_inner(const struct mg_connection* conn,
                                           lua_State* L) {
  const char* s;
  int i;
  ...
  if (conn->request_info.content_length >= 0) {
    ...
    lua_pushnumber(
        L, (lua_Number)conn->request_info
...
static void prepare_lua_request_info(const struct mg_connection* conn,
                                     lua_State* L) {
...
static void prepare_lua_response_table(struct mg_connection* conn,
                                       lua_State* L) {
  /* Export mg.request_info */
  lua_pushstring(L, "response");
  lua_newtable(L);
...
static void civetweb_open_lua_libs(lua_State* L) {
  {
 ...
 */
static int lsp_mg_gc(lua_State* L) {
  int context_flags;
  struct mg_context* ctx;
  struct mg_connection* conn =
      (struct mg_connection*)lua_touserdata(L, lua_upvalueindex(1));
...
static void reg_gc(lua_State* L, void* conn) {
  /* Key element */
...
static void prepare_lua_environment(struct mg_context* ctx,
  ...
  DEBUG_TRACE("Lua environment type %i: %p, connection %p, script %s",
              lua_env_type, L, conn, script_name);
  ...
  if (lua_env_type == LUA_ENV_TYPE_LUA_SERVER_PAGE) {
    ...
    lua_pushlightuserdata(L, (void*)&lua_regkey_lsp_include_history);
    h = (struct lsp_include_history*)lua_newuserdata(
  ...
  if ((conn != NULL) && (conn->dom_ctx != NULL)) {
    ...
    reg_string(L, "auth_domain", conn->dom_ctx->config[AUTHENTICATION_DOMAIN]);
#if defined(USE_WEBSOCKET)
    if (conn->dom_ctx->config[WEBSOCKET_ROOT]) {
      reg_string(L, "websocket_root", conn->dom_ctx->config[WEBSOCKET_ROOT]);
    } else {
      reg_string(L, "websocket_root", conn->dom_ctx->config[DOCUMENT_ROOT]);
    }
#endif
...
static void mg_exec_lua_script(struct mg_connection* conn, const char* path,
  ...
  if (path != NULL &&
      (L = lua_newstate(lua_allocator, (void*)(conn->phys_ctx))) != NULL) {
    prepare_lua_environment(conn->phys_ctx, conn, NULL, L, path,
...
static int handle_lsp_request(struct mg_connection* conn, const char* path,
                              struct mg_file* filep, struct lua_State* ls) {
  void* p = NULL;
  lua_State* L = NULL;
  struct lsp_include_history* include_history;
  int error = 1;
  int (*run_lsp)(struct mg_connection*, const char*, const char*, int64_t,
                 lua_State*, int);
  const char* addr;
  ...
  if (!mg_fopen(conn, path, MG_FOPEN_MODE_READ, filep)) {
    ...
    if (ls == NULL) {
      mg_send_http_error(conn, 500, "Error: Cannot open script file %s", path);
    } else {
  ...
  if ((p = mmap(NULL, (size_t)filep->stat.size, PROT_READ, MAP_PRIVATE,
                fileno(filep->access.fp), 0)) == NULL) {
    ...
    if (ls == NULL) {
      ...
      luaL_error(ls, "mmap(%s, %zu, %d): %s", path, (size_t)filep->stat.size,
                 fileno(filep->access.fp), strerror(errno));
    }
...
static void* lua_websocket_new(const char* script, struct mg_connection* conn) {
  ...
  if (*shared_websock_list == NULL) {
    /* add ws to list */
    *shared_websock_list = (struct mg_shared_lua_websocket_list*)mg_calloc_ctx(
        sizeof(struct mg_shared_lua_websocket_list), 1, conn->phys_ctx);
    if (*shared_websock_list == NULL) {
      conn->must_close = 1;
      mg_unlock_context(conn->phys_ctx);
      mg_cry_internal(conn, "%s", "Cannot create shared websocket struct, OOM");
      return NULL;
    }
    ...
    if (!ws->script) {
      conn->must_close = 1;
      mg_unlock_context(conn->phys_ctx);
      mg_cry_internal(conn, "%s", "Cannot create shared websocket script, OOM");
      return NULL;
    }
    ...
    ws->conn[0] = conn;
    ws->references = 1;
    prepare_lua_environment(conn->phys_ctx, conn, ws, ws->state, script,
...
static lua_State* mg_lua_context_script_prepare(const char* file_name,
                                                struct mg_context* ctx,
  ...
  if (L == NULL) {
    mg_snprintf(NULL, NULL, /* No truncation check for ebuf */
                ebuf, ebuf_len, "Error: %s", "Cannot create Lua state");
    return 0;
  }

  /* Add all libraries */
  prepare_lua_environment(ctx, NULL /* conn */, NULL /* WS list*/, L, file_name,
  ...
  if (lua_ret != LUA_OK) {
    ...
    mg_snprintf(NULL, NULL, /* No truncation check for ebuf */
                ebuf, ebuf_len, "Error loading file %s: %s\n", file_name,
                lua_err_txt);
    DEBUG_TRACE("Close Lua environment %p", L);
    lua_close(L);
    return 0;
...
static lua_State* mg_lua_context_script_run(lua_State* L, const char* file_name,
                                            struct mg_context* ctx, char* ebuf,
                                            size_t ebuf_len) {
  int lua_ret;
  const char* lua_err_txt;
  ...
  if (lua_ret != LUA_OK) {
    ...
    lua_err_txt = lua_tostring(L, -1);
    mg_snprintf(NULL, NULL, /* No truncation check for ebuf */
                ebuf, ebuf_len, "Error running file %s: %s\n", file_name,
                lua_err_txt);
    DEBUG_TRACE("Close Lua environment %p", L);
    lua_close(L);
    return 0;
  ...
  if (lua_isboolean(L, -1)) {
    ...
    if (ret == 0) {
      ...
      mg_snprintf(NULL, NULL, /* No truncation check for ebuf */
                  ebuf, ebuf_len, "Script %s returned false\n", file_name);
      DEBUG_TRACE("Close Lua environment %p", L);
      lua_close(L);
      return 0;
    }
...
static void lua_ctx_init(struct mg_context* ctx) {
#if defined(USE_WEBSOCKET)
  ctx->dd.shared_lua_websockets = NULL;
#endif
}

static void lua_ctx_exit(struct mg_context* ctx) {
#if defined(USE_WEBSOCKET)
  struct mg_shared_lua_websocket_list** shared_websock_list =
  ...
  mg_lock_context(ctx);
  while (*shared_websock_list) {
    DEBUG_TRACE("Close Lua environment %p", (*shared_websock_list)->ws.state);
...
static void lua_init_optional_libraries(void) {
...
static void lua_exit_optional_libraries(void) {
/* UUID library */
#if !defined(_WIN32)
  if (lib_handle_uuid) {
