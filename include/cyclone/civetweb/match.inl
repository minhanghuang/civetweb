...
static void match_context_push(const char* str, size_t len,
...
static ptrdiff_t mg_match_impl(const char* pat, size_t pat_len, const char* str,
                               struct mg_match_context* mcx) {
  /* Parse string */
  ...
  while (i_pat < pat_len) {
    ...
    if ((pat[i_pat] == '?') && (str[i_str] != '\0') && (str[i_str] != '/')) {
    ...
    if (pat[i_pat] == '*') {
      ...
      do {
        ret = mg_match_impl(pat + i_pat, (pat_len - (size_t)i_pat),
                            str + i_str + len, mcx);
      } while ((ret == -1) && (len-- > 0));
...
static ptrdiff_t mg_match_alternatives(const char* pat, size_t pat_len,
                                       const char* str,
                                       struct mg_match_context* mcx) {
...
static
#endif
ptrdiff_t mg_match(const char* pat, const char* str,
                   struct mg_match_context* mcx) {
  ...
  if (mcx != NULL) {
    if (ret < 0) {
      ...
      size_t elmsize = (size_t)(&mcx->match[1]) - (size_t)(&mcx->match[0]);
...
static ptrdiff_t match_prefix(const char* pattern, size_t pattern_len,
                              const char* str) {
  if (pattern == NULL) {
    return -1;
  }
